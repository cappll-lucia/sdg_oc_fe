...
import {
  createLineaVentaValidator,
  LineaVenta,
  NewLineaVentaType,
} from "./lineaVenta";
import {
  createMedioDePagoValidator,
  MedioDePago,
  NewMedioPagoType,
} from "./mediosDePago";
...
export const createVentaValidator = z
  .object({
    fecha: z
      .string()
      .datetime()
    ...
    lineasDeVenta: z
      .array(createLineaVentaValidator)
      .min(1, { message: "Ningun producto ha sido seleccionado" }),
    mediosDePago: z
      .array(createMedioDePagoValidator)
      .min(1, { message: "Indique al menos un medio de pago" }),
  })
  .superRefine((data, ctx) => {
    ...
    if (totalMediosPago !== totalVentaNeto) {
      ctx.addIssue({
        ...
        message: `El total de los medios de pago debe ser igual al total neto (${totalVentaNeto.toFixed(
          2
        )}).`,
        path: ["mediosDePago"],
      });
...
export const createVentaCustomValidator = (_newVenta: {
  ...
  const isValid = {
    ...
    descuentoPorcentaje:
      isValidNumber(_newVenta.descuentoPorcentaje) &&
      0 <= _newVenta.descuentoPorcentaje &&
      _newVenta.descuentoPorcentaje <= 100,
    condicionIvaVenta: _newVenta.condicionIvaVenta
      ? Object.values(CondicionIva).includes(
          Number(_newVenta.condicionIvaVenta)
        )
      : false,
  };
...
export const ventaObrasSocialesCustomValidator = (
  _ventasOS: {
  ...
  const isValidArray = _ventasOS.map((_os) => ({
    ...
    condicionIva: _os.condicionIva
      ? Object.values(CondicionIva).includes(Number(_os.condicionIva))
      : false,
  }));
