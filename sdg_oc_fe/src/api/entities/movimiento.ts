...
export const createMovimientoCtaCte = (_movimiento: {
  ...
  const entidadValida =
    !formaPagoValida || tiposNoRedValidation.includes(_movimiento.formaPago!)
      ? true
  ...
  const isValid = {
    tipoMovimiento: _movimiento.tipoMovimiento
      ? Object.keys(TipoMovimiento).includes(_movimiento.tipoMovimiento)
      : false,
