...
const loadData = async (id: string) => {
  try {
    ...
    currentVenta.value.cliente = await clientesApi.getOne(
      currentVenta.value.cliente.id
    );
  } catch (err: any) {
...
const montoDto = computed(() => {
  if (currentVenta.value) {
    return (
      (totalVentaFinal.value * currentVenta.value?.descuentoPorcentaje) / 100
    );
  } else return 0;
});
...
const printComprobante = async (
  ...
  try {
    ...
    a.href = url;
    a.download = `${currentVenta.value?.cliente.apellido}_${
      tipoComprobanteDisplay(tipoComprobante)?.nombre
    }_${fecha.toString().split("T")[0]}.pdf`;
    ...
    if (currentVenta.value) {
      const newFactura = {
        ...
        tipoComprobante: tipoFactura(
          Number(currentVenta.value?.cliente.categoriaFiscal)
        ),
        motivo: currentVenta.value?.observaciones,
        condicionIvaCliente: Number(
          currentVenta.value?.cliente.categoriaFiscal
        ),
    ...
    >
      ...
      >
        <div class="w-full flex flex-row items-center justify-start">
          <div class="w-[26rem] flex flex-col items-center justify-start">
            ...
            <div class="flex flex-row justify-start items-center w-full">
              ...
              <Label class="form-label w-[15rem] text-md">{{
                condicionIvaDisplay(currentVenta.cliente.categoriaFiscal)
              }}</Label>
            </div>
            <div class="flex flex-row justify-start items-center w-full">
              <Label class="form-label w-[11rem] text-md font-bold"
              ...
              <Label class="form-label w-[15rem] text-md">{{
                currentVenta?.fecha.toLocaleDateString("es-ES")
              }}</Label>
            </div>
          </div>
          <div class="flex flex-col items-center justify-end">
            <div
              v-if="currentVenta.factura"
              class="flex flex-col w-[6rem] h-[5rem] border justify-center items-center rounded-lg"
            >
              <span class="text-[1rem]">{{
                tipoComprobanteDisplay(currentVenta.factura.tipoComprobante)
                  ?.nombre
              }}</span>
              <span class="text-[2rem] leading-[2rem]">{{
                tipoComprobanteDisplay(currentVenta.factura.tipoComprobante)
                  ?.letra
              }}</span>
            </div>
        ...
        <div class="flex flex-col w-full justify-start items-start mt-10">
          ...
          <div
            v-for="(linea, index) in currentVenta?.lineasDeVenta"
            :key="index"
            class="linea-venta-item border-x border-b"
          >
            <span class="text-center id-input">{{
              linea.producto.codProv
            }}</span>
            <span class="text-left descripcion-input">{{
              linea.producto.descripcion
            }}</span>
            <span class="text-center"
              >$ {{ linea.precioIndividual.toFixed(2) }}</span
            >
            <span class="text-center">{{ linea.cantidad }}</span>
            <span class="text-center"
              >$
              {{ (linea.cantidad * linea.precioIndividual).toFixed(2) }}</span
            >
          </div>
          ...
          >
            ...
            >
              ...
              <div class="rounded-lg w-full min-h-[3rem]">
                ...
                <span v-if="!currentVenta.descuentoPorcentaje" class="text-sm"
                  >No aplica</span
                >
                <span v-else class="text-sm w-[12rem]"
                  >Dto Aplicado: {{ currentVenta.descuentoPorcentaje }} %</span
                >
              </div>
            </div>
            <div
            ...
            >
              <div
                v-if="
                  currentVenta.ventaObraSocial.length > 0 ||
                  currentVenta.descuentoPorcentaje
                "
              ...
              <div
                v-if="currentVenta.descuentoPorcentaje"
                class="flex justify-center items-center my-2"
              >
              ...
              <div class="flex justify-center my-2">
                ...
                <Label class="page-subtitle w-[10rem]"
        ...
        <div class="mt-6 w-full">
          ...
          <div
            v-for="medio in currentVenta.mediosDePago"
            class="w-full flex flex-row justify-start items-center px-[2rem] py-[0.5rem] border-x border-b"
          >
            <span class="text-[0.8rem] w-[15rem]">{{
              TipoMedioDePagoEnum[
                medio.tipoMedioDePago as keyof typeof TipoMedioDePagoEnum
              ]
            }}</span>
            <span class="text-[0.8rem] w-[10rem]"
              >$ {{ medio.importe.toFixed(2) }}</span
            >
            <span class="text-[0.8rem] w-[10rem]">{{
              RedDePago[medio.redDePago as keyof typeof RedDePago]
            }}</span>
            <span class="text-[0.8rem] w-[10rem]">{{
              medio.entidadBancaria
            }}</span>
          </div>
        </div>
        <div class="w-full mt-8">
          ...
          <span class="text-sm">{{
            currentVenta?.observaciones?.trim().length > 0
      ...
      <div
        class="w-full min-h-[10rem] flex flex-col justify-start my-12 items-start"
      >
        <div class="flex flex-row w-full mb-4 justify-between">
          ...
          <DropdownMenu v-if="currentVenta.factura">
            <DropdownMenuTrigger
              ><Button variant="outline" class="w-[14rem]"
                >Emitir nuevo comprobante</Button
              ></DropdownMenuTrigger
            >
            <DropdownMenuContent class="w-[14rem]">
              <DropdownMenuLabel
                ...
                @click="
                  router.push(`/nota-credito/new?venta=${currentVenta.id}`)
                "
                >Nueva nota de crédito</DropdownMenuLabel
              >
              <DropdownMenuSeparator />
              <DropdownMenuLabel
                class="cursor-pointer"
                @click="
                  router.push(`/nota-debito/new?venta=${currentVenta.id}`)
                "
                >Nueva nota de débito</DropdownMenuLabel
              >
            </DropdownMenuContent>
          </DropdownMenu>
        </div>
        <div class="flex flex-col justify-start w-full">
          ...
          <div class="border-x">
            ...
            <div v-if="comprobantesVenta.length">
              <div
                v-for="comprobante in comprobantesVenta"
                class="h-[4rem] flex justify-start items-center py-2 px-8 border-b"
              >
                <span class="text-md w-[8rem]">{{
                ...
                <span class="text-md mx-4 w-[11rem]"
                  ...
                  {{
                    tipoComprobanteDisplay(comprobante.tipoComprobante)?.letra
                  }}</span
                >
                <span class="text-md font-thin w-[15rem]">{{
                ...
                <div v-if="!loadingForm" class="actions">
                  ...
                  <Button
                    variant="ghost"
                    @click="emailComprobante(comprobante.id)"
                  >
                    <MailIcon />
                  </Button>
                  <Button
                    variant="ghost"
                    v-if="![1, 6].includes(comprobante.tipoComprobante)"
                    @click="
                      router.push(`/nota-credito-debito/view/${comprobante.id}`)
                    "
                  >
                    <InspectIcon />
                  </Button>
