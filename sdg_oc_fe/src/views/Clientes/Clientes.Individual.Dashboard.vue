...
const loadData = async () => {
  ...
  try {
    ...
    recetasCliente.value = await clientesApi.getRecetasSummaryByCliente(
      currentCliente.value.id
    );
    audiometriasCliente.value = await clientesApi.getAudiometriasByCliente(
      currentCliente.value.id
    );
    ctaCorriente.value = await cuentaCorrienteApi.getOneByCliente(
      currentCliente.value.id
    );
    ventasCliente.value = await ventasApi.getAllByCliente(
      currentCliente.value.id
    );
  } catch (err: any) {
...
const printComprobante = async (
  ...
  try {
    ...
    a.href = url;
    a.download = `${currentCliente.value?.apellido}_${
      tipoComprobanteDisplay(tipoComprobante)?.nombre
    }_${fecha.toISOString().split("T")[0]}.pdf`;
...
const onSubmit = async () => {
  try {
    ...
    if (currentCliente.value) {
      ctaCorriente.value = await cuentaCorrienteApi.updateSaldo(
        currentCliente.value?.id,
        newMovimiento.value
      );
      toast({
      ...
      <BreadcrumbList>
        ...
        <BreadcrumbItem>
          <BreadcrumbPage
            ...
            {{ currentCliente.nombre }}</BreadcrumbPage
          >
        </BreadcrumbItem>
      </BreadcrumbList>
    </Breadcrumb>
    <div class="pt-2">
      ...
      >
        ...
        >
          ...
          >
            <div class="flex flex-row items-center justify-start">
              <User class="rounded-[50%] bg-white p-2 mr-4 w-12 h-12" />
            ...
            <Button
              variant="outline"
              size="default"
              class="bg-transparent hover:bg-[#d7e5ec]"
          ...
          <div class="flex flex-col justify-center">
            <div
              class="w-full flex flex-row justify-start items-start h-[5rem]"
            >
              <div
                class="text-gray-700 w-[50%] h-[5rem] flex flex-col justify-between items-start"
              >
                ...
                <div class="flex flex-row justify-start">
                  ...
                  <span class="text-sm">{{
                    condicionIvaDisplay(currentCliente.categoriaFiscal)
                  }}</span>
                </div>
                <div class="flex flex-row justify-start">
              ...
              <div
                class="text-gray-700 w-[50%] h-[5rem] flex flex-col justify-between items-between"
              >
            ...
            <div
              class="flex flex-row mt-4 justify-start items-center h-[1rem] text-gray-700"
            >
              ...
              <div v-if="currentCliente.clienteObrasSociales.length">
                <Badge
                  ...
                  variant="outline"
                  class="font-normal border-gray-700 mr-4"
                  >{{ os.obraSocial.nombre }}</Badge
                >
              </div>
        ...
        >
          ...
          <div
            v-if="ctaCorriente"
            class="my-4 w-[90%] rounded-lg p-4 items-center flex flex-col bg-white"
            :class="{
              'border  ': (ctaCorriente?.saldo ?? 0) < 0,
              border: (ctaCorriente?.saldo ?? 0) >= 0,
            }"
          >
            ...
            <Label
              :class="{
                ' font-mono text-[2rem] text-red-700':
                  (ctaCorriente?.saldo ?? 0) < 0,
                ' text-[2rem] text-green-700': (ctaCorriente?.saldo ?? 0) >= 0,
              }"
              >$ {{ ctaCorriente?.saldo.toFixed(2) }}</Label
            >
          </div>
          <div
            v-else
            class="mt-4 w-[90%] rounded-lg p-4 items-center justify-center flex flex-col bg-white border h-20"
          >
            <Label class="font-mono text-center"
              >La cuenta corriente del cliente no ha sido creada
            </Label>
          </div>
        ...
        >
          <div class="flex flex-row justify-between">
            ...
            <div>
              <Button
                ...
                variant="outline"
                class="p-2 bg-transparent mr-4 hover:bg-white"
                ><Eye />
              </Button>
              <DropdownMenu>
                <DropdownMenuTrigger>
                  <Button
                    variant="outline"
                    class="p-2 bg-transparent hover:bg-white"
                    ><PlusIcon /> </Button
                ></DropdownMenuTrigger>
                <DropdownMenuContent class="w-[16rem] font-normal">
                  <DropdownMenuLabel
                    ...
                    @click="
                      router.push(
                        `/recetas/recetados/new?cliente=${currentCliente?.id}`
                      )
                    "
                    >Nueva receta anteojos recetados</DropdownMenuLabel
                  >
                  <DropdownMenuSeparator />
                  <DropdownMenuLabel
                    class="cursor-pointer font-normal"
                    @click="
                      router.push(
                        `/recetas/contacto/new?cliente=${currentCliente?.id}`
                      )
                    "
                    >Nueva receta lentes de contacto</DropdownMenuLabel
                  >
                </DropdownMenuContent>
              </DropdownMenu>
            </div>
          ...
          >
            <div
              ...
              class="cursor-pointer flex flex-row justify-center items-center h-[4rem] rounded-lg mx-2 my-4 mr-4"
            >
              <div
                class="cursor-pointer w-[39%] rounded-tl-lg h-full rounded-bl-lg border-r border font-semibold text-sm bg-secondary flex justify-end items-center"
              >
                <Label class="cursor-pointer p-4">
                  {{ formatDate(receta.fecha.toString()) }}
                </Label>
                <span class="w-[0px] mr-1"
                  ><CircleDot class="w-2 text-gray-700" />
                </span>
              </div>
              <div
                class="cursor-pointer w-[60%] h-full border-y border-r rounded-br-lg rounded-tr-lg pl-4 flex justify-start items-center"
              >
                <Label class="cursor-pointer text-sm font-thin"
                  >{{ receta.clase }}
                  <span v-if="receta.tipo">{{ receta.tipo }}</span>
                </Label>
              </div>
            </div>
          </ScrollArea>
          <div
            v-else
            class="mt-2 w-full flex flex-col items-center justify-center text-center h-[20rem] border rounded-lg bg-white py-4"
          >
            <Label class="text-sm"
              >El cliente no tiene recetas registradas</Label
            >
            <DropdownMenu>
              <DropdownMenuTrigger>
                <Button class="mt-4 text-xs w-36" variant="outline" size="sm"
                  >Nueva Receta</Button
                >
              </DropdownMenuTrigger>
              <DropdownMenuContent class="w-[16rem] font-normal">
          ...
          <div class="mt-8">
            <div class="flex flex-row justify-between">
              ...
              <div>
                ...
                <Button
                  @click="
                    router.push(
                      `/audiometrias/create?cliente=${currentCliente?.id}`
                    )
                  "
                  variant="outline"
                  class="p-2 bg-transparent hover:bg-white"
            ...
            <ScrollArea
              v-if="audiometriasCliente.length"
              class="mt-2 w-full h-[20.5rem] pr-4 rounded-lg bg-white border"
            >
              <div
                v-for="(audiom, index) in audiometriasCliente"
                @click="router.push(`/audiometrias/${currentCliente?.id}`)"
                class="cursor-pointer flex flex-row justify-start items-center h-[3rem] p-4 rounded-lg bg-secondary border mx-2 my-4 mr-4"
              >
                <Label class="cursor-pointer mr-4 text-sm">
                  {{ formatDate(audiom.fechaInforme.toString()) }}
                </Label>
                <div class="cursor-pointer flex flex-col">
                  <Label class="cursor-pointer text-sm font-thin"
                    >Audiometía {{ index }}</Label
                  >
                </div>
              </div>
            </ScrollArea>
            <div
              v-else
              class="mt-2 w-full text-center flex flex-col items-center justify-center h-[20.5rem] border rounded-lg bg-white py-4"
            >
              <Label class="text-sm"
                >El cliente no tiene audiometrías registradas</Label
              >
              <Button
                @click="
                  router.push(
                ...
                "
                class="mt-4 text-xs w-36"
                variant="outline"
                size="sm"
                >Nueva Audiometría</Button
              >
            </div>
          </div>
        </div>
              ...
              <div v-for="venta in ventasCliente">
                ...
                >
                  <div class="flex flex-row justify-start items-center">
                    ...
                    <Label class="w-[13rem] text-sm flex justify-start">
                      <Label
                        :class="[
                    ...
                    <Label
                      ...
                      class="w-[15rem] text-xs flex justify-start"
                      >{{
                        venta.factura?.motivo ?? "Venta no facturada"
                      }}</Label
                    >
                    <Label v-else class="w-[15rem] text-xs flex justify-start"
                      >Venta</Label
                    >
                    <Label class="w-[8rem] text-sm">
          ...
          >
            <div class="flex flex-row justify-between">
              ...
              <Dialog v-model:open="openDialog">
                  <Button variant="outline" type="button" @click="handleNewMovimiento"  > Nuevo Movimiento </Button>
                  ...
                  <form @submit.prevent="validateAndSubmit" v-if="!loadingForm">
                    <div class="grid gap-4 py-4">
                      ...
                      >
                        ...
                        <ScrollArea v-if="ctaCorriente?.movimientos.length" class=" mt-2 p-4 w-full h-full rounded-lg bg-white border">
                            <div v-for="mov in ctaCorriente.movimientos" >
                                <div class="flex flex-row justify-between w-full  mb-2 border-b items-center px-4 h-[4rem] ">
                                    <div class="flex flex-row justify-start items-center">
                                        <Label class="w-[8rem] text-sm ">{{ formatDate(mov.fechaMovimiento) }}</Label>
                                        <Label class="w-[13rem] text-sm flex justify-start "> <Label class="bg-secondary px-4 py-2 rounded-lg">{{mov.tipoMovimiento}}</Label></Label>
                                        <Label class="w-[15rem] text-xs flex justify-start ">Cuenta Corriente</Label>
                                        <Label class="w-[8rem] text-sm"><span class="font-bold" v-if="mov.tipoMovimiento==TipoMovimiento.VENTA">-</span> $ {{ mov.importe.toFixed(2)}}</Label>
                                    </div>      
                                </div>
                        ...
                        <div v-else class=" mt-2 w-full text-center flex justify-center items-center h-full border rounded-lg bg-white py-4">
                                <Label class="text-sm ">No hay movimientos de cuenta corriente registrados para el cliente </Label>   
                        </div>
                    </div>
                </div>
            </div>
