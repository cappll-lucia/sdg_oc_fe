    ...
    <div class="page"  id="dashboard" v-if="currentCliente">
        ...
        <div class="pt-2">
            <div v-if="currentCliente" class="w-full flex flex-row justify-between h-[16rem]">
                <div class="w-[72%] bg-secondary rounded-lg px-10 py-5 flex flex-col justify-evenly">
                    <div class="flex flex-row  w-full items-center justify-between h-[3rem]  ">
                ...
                <div class="w-[25%] bg-secondary rounded-lg p-4 flex flex-col justify-center items-center">
                        ...
                        <div v-if="ctaCorriente" class=" my-4 w-[90%] rounded-lg p-4 items-center flex flex-col bg-white" :class="{'border  ': (ctaCorriente?.saldo ?? 0) < 0, 'border': (ctaCorriente?.saldo ?? 0) >= 0}">
            ...
            <div class="w-full flex flex-row h-[50rem] justify-between items-start mt-8 gap-x-4">
                <div class="w-[30%] h-full bg-secondary rounded-lg p-4 flex flex-col justify-start">
                        ...
                        <ScrollArea v-if="audiometriasCliente.length" class=" mt-2 w-full h-[20.5rem] pr-4  rounded-lg bg-white border">
                        ...
                        <div v-else class=" mt-2 w-full text-center flex flex-col items-center justify-center h-[20.5rem] border rounded-lg bg-white py-4">
                ...
                <div class="flex flex-col justify-between w-[70%] h-full">
                    <div class="h-[50%] w-full bg-secondary rounded-lg p-4 flex flex-col justify-start">
                        ...
                        <ScrollArea v-if="comprobantesCliente.length" class=" mt-2 p-4 w-full h-full rounded-lg bg-white border">
                            <div v-for="comprobante in comprobantesCliente" >
                                <div  class="flex flex-row justify-between w-full  mb-2 border-b items-center px-4 h-[4rem] ">
                                    <div class="flex flex-row justify-start items-center">
                                        <Label class="w-[8rem] text-sm ">{{ formatDate(comprobante.fechaEmision) }}</Label>
                                        <Label class="w-[13rem] text-sm flex justify-start "> <Label class="bg-secondary px-4 py-2 rounded-lg">{{ tipoComprobanteDisplay(comprobante.tipoComprobante)?.nombre.toLocaleUpperCase() }} {{ tipoComprobanteDisplay(comprobante.tipoComprobante)?.letra }}</Label></Label>
                                        <Label v-if="![1,6].includes(comprobante.tipoComprobante)" class="w-[15rem] text-xs flex justify-start ">{{ comprobante.motivo }}</Label>
                                        <Label v-else class="w-[15rem] text-xs flex justify-start ">Venta</Label>
                                        <Label class="w-[8rem] text-sm "> $ {{ comprobante.importeTotal.toFixed(2)}}</Label>
                                    </div>
                                    <div class="flex flex-row ">
                                        ...
                                        <Button v-if="[1, 6].includes(comprobante.tipoComprobante) && comprobante.venta" variant="ghost" @click="router.push(`/ventas/view/${comprobante.venta.id}`)">
                                            <InspectIcon />
                                        </Button>
                                        <Button v-if="[3,8,2,7].includes(comprobante.tipoComprobante)" variant="ghost" @click="router.push(`/nota-credito-debito/view/${comprobante.id}`)">
                                            <InspectIcon />
                                        </Button>
                                    </div>
                                </div>
                        ...
                        <div v-else class=" mt-2 w-full text-center flex justify-center items-center h-full border rounded-lg bg-white py-4">
                                <Label class="text-sm ">No hay ventas ni comprobantes registrados para el cliente </Label>   
                        </div>
                    </div>
                    <div class="h-[46%] w-full bg-secondary rounded-lg p-4 flex flex-col justify-start">
                        <div class=" flex flex-row justify-between">
