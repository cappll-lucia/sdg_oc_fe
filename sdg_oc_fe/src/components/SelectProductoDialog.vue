  ...
  <Dialog :open="modelValue" @update:open="emit('update:modelValue', $event)">
    <DialogContent class="w-[60rem] h-[45rem] max-w-[60rem] px-8">
      ...
      <div class="flex flex-row justify-between px-6 h-2 mb-4">
        <Input
          type="text"
          v-model="searchProdCodTxt"
          class="w-[23%] mr-4 text-center"
          placeholder="Código"
          @keyup.enter="handleFilterProducts()"
          @input="(e: any) => e.target.value.length >= 0 && handleFilterProducts()"
        />
        <Input
          type="text"
          v-model="searchProdDscTxt"
          class="mr-4"
      ...
      <ScrollArea v-if="!loadingForm" class="h-[30rem] w-[51rem] pl-4 mt-4">
        ...
        >
          <div class="w-[85%] flex flex-row justify-start items-center">
            <Label class="w-[23%] text-sm text-center">{{
              prod.codProv
            }}</Label>
